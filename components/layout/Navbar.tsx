...
export default function Navbar() {
  ...
  useEffect(()=>{
    ...
    sb.auth.getUser().then(({data:{user}})=>{
      setUser(user)
      if(user) sb.from('profiles').select('full_name,is_premium,avatar_url').eq('id',user.id).maybeSingle().then(({data})=>setProfile(data))
    })
    ...
    <nav style={{position:'sticky',top:0,zIndex:100,background:'rgba(255,255,255,0.97)',backdropFilter:'blur(8px)',borderBottom:'1px solid #e2e8f0'}}>
              ...
              {drop && (
                <div style={{position:'absolute',right:0,top:'calc(100% + 6px)',background:'#fff',borderRadius:'12px',boxShadow:'0 8px 30px rgba(0,0,0,0.12)',border:'1px solid #e2e8f0',minWidth:'180px',zIndex:200}}>
                  {[{href:'/dashboard',label:'Dashboard'},{href:'/dashboard/profile',label:'Edit Profile'},{href:'/pricing',label:'Upgrade'}].map(item=>(
                    <Link key={item.href} href={item.href} onClick={()=>setDrop(false)} style={{display:'block',padding:'11px 16px',fontSize:'14px',color:'#374151',fontWeight:'500',borderBottom:'1px solid #f8fafc'}}>{item.label}</Link>
                  ))}
      ...
      {open && (
        <div style={{background:'#fff',borderTop:'1px solid #e2e8f0',padding:'12px 20px'}}>
          {links.map(l=>(
            <Link key={l.href} href={l.href} onClick={()=>setOpen(false)} style={{display:'block',padding:'10px 0',fontSize:'15px',color:'#374151',fontWeight:'500',borderBottom:'1px solid #f8fafc'}}>{l.label}</Link>
          ))}
          {user
