...
export default function HomePage(){
  ...
  const [gender,setGender]=useState('Auto')  // Auto = show opposite gender
  ...
  useEffect(()=>{
    ...
    createClient().auth.getUser().then(({data:{user}})=>{
      if(user){
        ...
        // Set filter to show opposite gender automatically
        ...
        createClient().from('likes').select('sender_id').eq('receiver_id',user.id)
          .then(({data:recv})=>{
            ...
            createClient().from('likes').select('receiver_id').eq('sender_id',user.id)
              .then(({data:sent})=>{
                if(!sent) return
                setMatches(new Set(sent.filter((l:any)=>rIds.has(l.receiver_id)).map((l:any)=>l.receiver_id)))
              })
          })
      }
    })
    ...
    if(navigator.geolocation)
      navigator.geolocation.getCurrentPosition(p=>setUserLocation({lat:p.coords.latitude,lng:p.coords.longitude}),()=>{},{enableHighAccuracy:true,timeout:10000})
