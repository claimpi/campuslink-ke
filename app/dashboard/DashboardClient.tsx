...
export default function DashboardClient() {
  ...
  useEffect(()=>{
    async function load(){
      ...
      // Try to get profile - use id (matches old schema)
      ...
      if (!profile) {
        ...
        if (!createErr) setProfile(newProfile)
      } else {
        setProfile(profile)
      }
      setLoading(false)
  ...
  return(
    <div style={{maxWidth:'1100px',margin:'0 auto',padding:'32px 16px'}}>
      {profileError && (
        <div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:'12px',padding:'12px 16px',marginBottom:'16px',color:'#dc2626',fontSize:'13px'}}>
          ⚠️ Profile error: {profileError} — <a href="/dashboard/profile" style={{color:'#dc2626',fontWeight:'700'}}>Try editing your profile</a>
        </div>
      )}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:'14px',marginBottom:'24px'}}>
        {[
          ...
          <div key={s.label} style={{background:'white',borderRadius:'16px',border:'1px solid #f3f4f6',padding:'18px',boxShadow:'0 2px 8px rgba(0,0,0,0.05)'}}>
            ...
            <div style={{fontSize:'18px',fontWeight:'900',color:'#111827',marginBottom:'2px'}}>{String(s.value)}</div>
      ...
      <div style={{background:'white',borderRadius:'20px',border:'1px solid #f3f4f6',padding:'22px',marginBottom:'22px',boxShadow:'0 2px 8px rgba(0,0,0,0.05)'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:'10px'}}>
          ...
          <span style={{fontSize:'14px',color:'#f97316',fontWeight:'800'}}>{pct}%</span>
        </div>
        <div style={{background:'#f3f4f6',borderRadius:'50px',height:'8px',overflow:'hidden',marginBottom:'14px'}}>
          <div style={{height:'100%',background:'linear-gradient(135deg,#f97316,#ea580c)',borderRadius:'50px',width:`${pct}%`,transition:'width 0.6s'}}/>
        </div>
        <div style={{display:'flex',flexWrap:'wrap',gap:'7px'}}>
          {[{l:'University',d:!!profile?.university},{l:'Course',d:!!profile?.course},{l:'WhatsApp',d:!!profile?.whatsapp_number},{l:'Bio',d:!!profile?.bio},{l:'Interests',d:(profile?.interests||[]).length>0}].map(x=>(
            <span key={x.l} style={{fontSize:'12px',padding:'4px 10px',borderRadius:'50px',background:x.d?'#f0fdf4':'#f9fafb',color:x.d?'#16a34a':'#9ca3af',border:`1px solid ${x.d?'#bbf7d0':'#e5e7eb'}`,fontWeight:'600'}}>{x.d?'✓':'○'} {x.l}</span>
          ))}
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))',gap:'12px',marginBottom:'24px'}}>
        {[{href:'/dashboard/profile',emoji:'✏️',title:'Edit Profile',desc:'Update your info',border:'#fed7aa'},{href:'/discover',emoji:'🔍',title:'Browse Students',desc:'Find and connect',border:'#bfdbfe'},{href:'/groups',emoji:'💬',title:'WhatsApp Groups',desc:'Join university groups',border:'#bbf7d0'},{href:'/pricing',emoji:'👑',title:'Go Premium',desc:'Unlock all features',border:'#c4b5fd'}].map(a=>(
